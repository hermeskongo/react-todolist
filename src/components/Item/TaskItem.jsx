...
export const TaskItem = ({
    ...
    return (
        <li
            className={`${ task?.completed ? style.success : style.default} ${style.container}`}
            onClick={handleEditTask}
        >
            <div style={includedStyle}>
                <div className={style.item}>
                    <div className={`${style.id} ${ task?.completed ? style.idSuccess : style.idDefault}`}>{task.id}</div>
                </div>
                <div className={`${ task?.completed ? style.contentSuccess : style.contentDefault}`}>
                    {task.title}
                </div>
            </div>
            <button className="button-primary" onClick={handleDeleteTask}>
